...
public class CancionImpl implements Cancion{
	...
	//Constructor1
	public CancionImpl(String id,String nombre,String url,Integer popularidad,Artista Interprete,Duration duracion, Artista interprete) {
	...
	public CancionImpl(String s) {
		...
		//Duration duracion = new Duration(new Long(trozos[4].trim()),new Integer(trozos[5].trim()));
	...
	public String getResumenCancion() {
		String res = this.getNombre() + " - "
				+ this.getInterprete().getNombre() + " ("
				+ this.getDuracionString() + ")";
		return res;
	}
	@Override
